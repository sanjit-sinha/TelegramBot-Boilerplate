...
async def update(client, message: Message):
    ...
    await message.reply_animation(animation='https://telegra.ph/file/fd2495f0465f5293bd052.mp4', caption=f"**≧◉◡◉≦ Bot is Up and Running successfully.**\n\n× Bot Uptime: `{currentTime}`\n× Total Disk Space: `{total}`\n× Used: `{used}({disk_usage}%)`\n× Free: `{free}`\n× CPU Usage: `{cpu_usage}%`\n× RAM Usage: `{ram_usage}%`",  quote=True)
